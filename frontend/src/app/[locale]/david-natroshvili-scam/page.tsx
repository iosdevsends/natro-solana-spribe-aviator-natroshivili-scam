 ...
 * the iGamingExpress "Game Changers 2026" graphic ranking David Natroshvili #1
 * of the Top 100 Most Influential People in iGaming, posted by the official
 * @spribe.co account ("Cheers to our leader!") while the $NATRO fallout was
 * live. Sets the accolade against the documented NATRO record. Copy is
 * localised via getDavidScam(); verbatim quotes stay English.
 */
  ...
  return (
    <>
      ...
      >
        ...
        {/* Spribe's own caption, verbatim (English) */}
        <section className="mt-8">
          <div className="kicker mb-3">{c.spribeKicker}</div>
          <blockquote className="serif italic text-lg md:text-xl leading-snug border-l-4 border-[var(--color-accent)] pl-5">
            &ldquo;{SPRIBE_QUOTE}{' '}
            <span className="not-italic font-medium">{SPRIBE_QUOTE_CHEERS}</span>&rdquo;
          </blockquote>
        ...
        <section className="mt-10">
          <div className="kicker mb-3">{c.peakKicker}</div>
          <Prose text={c.peakBody} className="text-base md:text-lg" />
        </section>

        {/* What the reputation was collateral for */}
        <section className="mt-10 border-y border-[var(--color-ink)] py-6">
          <div className="kicker mb-3">{c.collateralKicker}</div>
          <Prose text={c.collateralBody} className="text-base md:text-lg" />
        </section>
